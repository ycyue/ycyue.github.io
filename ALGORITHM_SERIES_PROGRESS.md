...
|---:|---|---|---:|---|---|---|
| Map | Python 基础算法学习地图 | 完整版 | 1 | N/A | 已完成 | 已上线验证 |
| 01 | 相向双指针（一） | 已完成 | 2 | 已通过 | 已完成 | 已上线验证 |
| 02–05 | 数组与基础算法思想 | 已完成 | 8 | 已通过 | 已完成 | 已上线验证 |
| 06–08 | 链表 | 已完成 | 6 | 已通过 | 已完成 | 已上线验证 |
| 09–13 | 二叉树 | 已完成 | 10 | 已通过 | 已完成 | 已上线验证 |
| 14–16 | 回溯 | 已完成 | 6 | 已通过 | 已完成 | 已上线验证 |
| 17–22 | 动态规划 | 已完成 | 12 | 已通过 | 已完成 | 已上线验证 |
| 23–25 | 树形 DP | 已完成 | 6 | 已通过 | 已完成 | 已上线验证 |
| 26–27 | 单调数据结构 | 已完成 | 4 | 已通过 | 已完成 | 已上线验证 |

## 当前结论

27 篇教程与 1 篇学习地图已经全部发布。后续工作只包括根据实际学习反馈修订例子、补充练习或修正文案，不再有待发布批次。

## 已发现问题与处理

- Mermaid 未接入：采用本地原创 SVG；不引入外部图片依赖。
- 根目录同时保存构建产物和源码：只编辑 `source/**`、构建脚本与项目状态文件，生成产物交给既有工作流更新。
- 制作过程没有提前放出空链接；全部正文完成后，学习地图才一次性接入 27 篇真实页面。
- 全系列已通过 Hexo 构建、内部链接、Python 样例、SVG XML、深色模式、代码横向滚动、学习地图 27 行和阅读进度检查；640px 响应式断点覆盖 360px 布局规则，图、表、代码块和三栏导航均有窄屏保护。
- 内容提交 `b4df2b8b39d96f36b2cb379ca2dbc9f1419af335` 已于 2026-09-04（Asia/Shanghai）推送；Publish blog 与 Pages deployment 均成功，自动构建提交为 `b19c946dceb8a070e6f5ec059099c95c8fbf9498`。
- 线上检查覆盖 28 个页面、55 张 SVG、系列 CSS 与阅读脚本；页面与资源均返回 HTTP 200。按模块抽查的 10 篇文章均正常，末篇在线滚动进度为 100%，深色模式颜色和 SVG 滤镜生效，未出现横向溢出。
- 02–27 的正文与 52 张 SVG 由 `tools/algorithm-series-data.mjs` 中的逐课教学数据生成，生成入口为 `tools/generate-algorithm-series.mjs`；发布内容仍是独立 Markdown 和本地 SVG，可在中断后完整复现。
