...
经典主线完成后，系列继续补充面试与刷题中高频、但不在上述 27 节主线中的基础工具。

| # | 主题 | 教程重点 | 数据结构 | 代表题 | 前置知识 | 直接依赖 |
|---:|---|---|---|---|---|---|
| 28 | 前缀和与差分 | 区间和、边界抵消、批量区间修改 | 数组 | 303、724、560、1109 | 列表、循环、下标 | 无 |
...
| 28 | 前缀和与差分（扩展篇） | 已完成 | 2 | 已通过 | 已完成 | 已上线验证 |

## 当前结论

27 篇经典主线、学习地图与第 28 课“前缀和与差分”均已发布。后续扩展篇可以继续按“正文、两张原创 SVG、代码测试、地图接入、线上验证”的完整标准逐课增加。

## 已发现问题与处理

- Mermaid 未接入：采用本地原创 SVG；不引入外部图片依赖。
- 根目录同时保存构建产物和源码：只编辑 `source/**`、构建脚本与项目状态文件，生成产物交给既有工作流更新。
- 制作过程没有放出空链接；学习地图只接入已经具备完整正文、图解与测试的真实页面。
- 全系列本地检查已扩展到 28 篇教程与 57 张 SVG，并覆盖 Hexo 构建、内部链接、Python 样例、SVG XML、深色模式、代码横向滚动、学习地图 28 行和阅读进度；640px 响应式断点覆盖 360px 布局规则，图、表、代码块和三栏导航均有窄屏保护。
...
- 第 28 课内容提交为 `139ca7f00754eee60b649fb5663129633939ab2d`，自动构建提交为 `e2c333c786e4ee3e201c22a098d1bd91ec56bae6`；Publish blog 与 Pages deployment 均成功。
- 线上检查已扩展到 29 个系列页面、57 张 SVG、系列 CSS 与阅读脚本；第 28 课、两张新 SVG、学习地图入口及 27→28 导航均返回 HTTP 200。
- 02–28 的正文与 54 张 SVG 由 `tools/algorithm-series-data.mjs` 中的逐课教学数据生成，生成入口为 `tools/generate-algorithm-series.mjs`；发布内容仍是独立 Markdown 和本地 SVG，可在中断后完整复现。
